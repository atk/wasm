...
const setBuffer = (input) => {
  inputLengthEncoded = index.length * 4 /* i32 = 4 bytes */;
  if (inputLengthEncoded > inputBufferCapacity)
    throw new Error(`Array is too large for the ${inputBufferCapacity} bytes memory slice`);
  const buffer = currentInstance.get_mem_as_i32(inputBufferOffset, inputLengthEncoded);
  buffer.set(input, 0);
  return [inputBufferOffset, inputLengthEncoded];
}

const getItem = (input, index) => {
  const [inputOffset, inputLength] = setBuffer(input);
  return currentInstance.exports.getItem(inputOffset, inputLength, index);
}

const setItem = (input, index, item) => {
  const [inputOffset, inputLength] = setBuffer(input);
  currentInstance.exports.setItem(inputOffset, inputLength, index, item);
  return [...currentInstance.get_mem_as_i32(inputOffset, inputEncoded)];
}

const insertItemAtTop = (input, item) => {
  const [inputOffset, inputLength] = setBuffer(input);
  const [outputOffset, outputLength] = currentInstance.exports.insertItemAtTop(inputOffset, inputLength, item);
  const output = [...currentInstance.get_mem_as_i32(outputOffset, outputLength)];
  return Object.assign(input, output, { length: output.length });
}

const insertItemAtBottom = (input, item) => {
  const [inputOffset, inputLength] = setBuffer(input);
  const [outputOffset, outputLength] = currentInstance.exports.insertItemAtBottom(inputOffset, inputLength, item);
  const output = [...currentInstance.get_mem_as_i32(outputOffset, outputLength)];
  return Object.assign(input, output, { length: output.length });
}
...
const removeItem = (input, index) => {
  ...
  const [inputOffset, inputLength] = setBuffer(input);
  const [outputOffset, outputLength] = currentInstance.exports.removeItem(inputOffset, inputLength, index);
  const output = [...currentInstance.get_mem_as_i32(outputOffset, outputLength)];
  return Object.assign(input, output, { length: output.length });
} 

const removeItemFromTop = (input) => {
  const [inputOffset, inputLength] = setBuffer(input);
  const [outputOffset, outputLength] = currentInstance.exports.removeItemFromTop(inputOffset, inputLength);
  const output = [...currentInstance.get_mem_as_i32(outputOffset, outputLength)];
  return Object.assign(input, output, { length: output.length });
}

const checkSizeOfStack = (input, size) => {
  const [inputOffset, inputLength] = setBuffer(input);
  return Boolean(currentInstance.exports.checkSizeOfStack(inputOffset, inputLength, size));
}
