...
const frontDoorResponse = (input) => {
  const [offset, length] = writeToMemory(input);
  const [outputOffset, outputLength] = currentInstance.exports.frontDoorResponse(offset, length);
  return currentInstance.get_mem_as_utf8(outputOffset, outputLength);
}

const frontDoorPassword = (input) => {
  const [offset, length] = writeToMemory(input);
  const [outputOffset, outputLength] = currentInstance.exports.frontDoorPassword(offset, length);
  return currentInstance.get_mem_as_utf8(outputOffset, outputLength);
}

const backDoorResponse = (input) => {
  const [offset, length] = writeToMemory(input);
  const [outputOffset, outputLength] = currentInstance.exports.backDoorResponse(offset, length);
  return currentInstance.get_mem_as_utf8(outputOffset, outputLength);
}

const backDoorPassword = (input) => {
  const [offset, length] = writeToMemory(input);
  const [outputOffset, outputLength] = currentInstance.exports.backDoorPassword(offset, length);
  return currentInstance.get_mem_as_utf8(outputOffset, outputLength);
}
